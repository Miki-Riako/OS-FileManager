...
bool FileSystemCore::createDisk(uint32_t sz) {
    bool ok = disk.init(sz);
    return ok;
}

bool FileSystemCore::format(uint16_t bsize) {
    if (!isOpen) {
        return false;
    }
...
//分配块，从空闲块栈中获取一个空闲块
uint32_t FileSystemCore::blockAllocate() {
...
//释放块，将块归还到空闲块栈
void FileSystemCore::blockFree(uint32_t bno) {
...
//读取块数据
void FileSystemCore::read(uint32_t bno, uint16_t offset, char* buf, uint16_t sz) {
...
//写入块数据
void FileSystemCore::write(uint32_t bno, uint16_t offset, const char* buf, uint16_t sz) {
    uint32_t base = bno * blockSize; //计算块的起始地址
    disk.seekStart(base + offset); //定位到块的具体偏移
    disk.write(buf, sz); //写入数据
}

void FileSystemCore::readNext(char* buf, uint16_t sz) {
    disk.read(buf, sz); //从磁盘读取指定大小的数据到缓冲区
}

void FileSystemCore::writeNext(char* buf, uint16_t sz) {
    disk.write(buf, sz); //将缓冲区中的数据写入磁盘
}

void FileSystemCore::locale(uint32_t bno, uint16_t offset) {
    uint32_t base = bno * blockSize; //计算块号对应的字节偏移量
    disk.seekStart(base + offset); //定位磁盘读写指针到指定位置
}

uint8_t FileSystemCore::userVerify(std::string userName, std::string password) {
    for (uint8_t i = 0; i < MAX_USER_NUMS; ++i) {
...
uint8_t FileSystemCore::duplicateDetection(std::string userName) {
...
int8_t FileSystemCore::emptyDetection() {
    for (int8_t i = 0; i < MAX_USER_NUMS; i++) {
        if (!systemInfo.users[i].uid) {
...
void FileSystemCore::mkuser(int8_t idx, std::string name, std::string password) {
...
void FileSystemCore::rmuser(uint8_t uid) {
...
void FileSystemCore::passwd(uint8_t uid, std::string password) {
    int8_t idx = uid - 1; //根据用户ID计算索引
    strcpy(systemInfo.users[idx].password, password.c_str()); //复制新的密码
    systemInfo.flag = 1; //标记系统信息已修改
}

bool FileSystemCore::grantTrustUser(std::string currentUser, std::string targetUser) {
...
bool FileSystemCore::revokeTrustUser(std::string currentUser, std::string targetUser) {
    int8_t curIdx = -1, tarIdx = -1;
    for (int8_t i = 0; i < MAX_USER_NUMS; ++i) {
        if (!systemInfo.users[i].uid) {
            continue; //跳过没有用户ID的条目
...
uint8_t FileSystemCore::verifyTrustUser(std::string currentUser, std::string targetUser) {
    int8_t curIdx = -1, tarIdx = -1;
    for (int8_t i = 0; i < MAX_USER_NUMS; ++i) {
        if (!systemInfo.users[i].uid) {
            continue; //跳过没有用户ID的条目
...
uint8_t FileSystemCore::verifyTrustUser(uint8_t currentUserUid, uint8_t targetUserUid) {
    int8_t curIdx = currentUserUid - 1, tarIdx = targetUserUid - 1; //计算用户索引
    return systemInfo.trustMatrix[curIdx][tarIdx]; //返回信任关系的状态
}

void FileSystemCore::getUser(uint8_t uid, User* user) {
    int8_t idx = uid - 1; //根据用户ID计算索引
